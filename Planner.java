...
Submodules: LinkedUnits
            composed of PlannerUnits
...
public class Planner {
    ...
    public void setAllUnits(LinkedUnits n) {
        allUnits = n;
    }
    ...
    public void orderChornologically() {
        ...
        for(int pass = 1; (pass<allUnits.size()) && !sorted; ++pass) {
            ...
            for(int index = 0; index<allUnits.size()-pass; ++index) {
                ...
                if(allUnits.findByIndex(index).sortValue() >
                        allUnits.findByIndex(nextIndex).sortValue()) {
                    // if former is greater than latter, exchange items
                    allUnits.swap(allUnits.findByIndex(index),
                            allUnits.findByIndex(nextIndex));
